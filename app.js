...
const ddEl = document.querySelector('.circle')
...
function timeSet() {
    ...
    timeEl.textContent = `${hour < 10 ? `0${hour}`: hour}:${minute < 10? `0${minute}`: minute}:${second < 10 ? `0${second}` : second}`;


    dateEl.textContent = `${days[day]}, ${months[month]} ${ddEl.textContent = date}`
